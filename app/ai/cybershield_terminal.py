...
ALIASES = {
    ...
    "dns-resolve": {"dns-resolve", "dns", "dns-tekshir", "resolve"},
    "port-check": {"port-check", "port", "tcp-port", "port-tekshir"},
    "listening-risk": {"listening-risk", "listening", "ochiq-portlar", "listen-risk"},
    "security-baseline": {"security-baseline", "security-baseline-check", "xavfsizlik-bazasi"},
    "process-network": {"process-network", "process-network-view", "jarayon-tarmoq"},
    ...
    def execute(self, line: str) -> str:
        ...
        try:
            ...
            if command == "dns-resolve":
                if not args: raise CommandError("Usage: dns-resolve <host>")
                return _json(procmd.dns_resolve(args[0]))
            if command == "port-check":
                if len(args) < 2: raise CommandError("Usage: port-check <host> <port>")
                return _json(procmd.port_check(args[0], int(args[1])))
            if command == "listening-risk":
                return _json(procmd.listening_risk(int(args[0]) if args else 50))
            if command == "security-baseline": return _json(procmd.security_baseline())
            if command == "process-network":
                return _json(procmd.process_network(int(args[0]) if args else 50))
