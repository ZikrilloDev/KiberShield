...
def github_publish(root: Path, repo: str | None = None, private: bool = True) -> dict:
    ...
    # Existing repository: always align origin with the requested GitHub target.
    # The previous version kept an old origin, so `github publish <new-url>` could
    # silently push to the wrong repository and then fall through to strategy 4.
    if _git_repo(root):
        desired = f"https://github.com/{target}.git" if target else None
        remote = _run(["git", "remote", "get-url", "origin"], root, timeout=30)
        attempts.append({"strategy": 1, "name": "existing-origin-check", "result": remote})
        if desired:
            if not remote["ok"]:
                set_remote = _run(["git", "remote", "add", "origin", desired], root, timeout=30)
            elif remote["stdout"].strip().rstrip("/") != desired.rstrip("/"):
                set_remote = _run(["git", "remote", "set-url", "origin", desired], root, timeout=30)
            else:
                set_remote = {"ok": True, "code": 0, "stdout": remote["stdout"], "stderr": ""}
            attempts.append({"strategy": 1, "name": "align-origin", "result": set_remote})
            if set_remote["ok"]:
                ready = _ensure_git_commit(root, attempts)
                if ready["ok"]:
                    push = _run(["git", "push", "-u", "origin", "HEAD:main"], root, timeout=240)
                    attempts.append({"strategy": 1, "name": "verified-git-push", "result": push})
                    if push["ok"]:
                        verify = _run(["git", "ls-remote", "--heads", "origin", "main"], root, timeout=60)
                        attempts.append({"strategy": 1, "name": "remote-verification", "result": verify})
                        if verify["ok"] and verify["stdout"].strip():
                            return {"ok": True, "passed_strategy": 1, "repository": target, "attempts": attempts}
    ...
    if gh:
        ...
        # `gh repo create --source` is used only when creation is needed.
        ...
        if create["ok"]:
            ...
            # If the repository already exists, attach origin and push to it.
            remote_url = f"https://github.com/{name}.git"
            set_remote = _run(["git", "remote", "set-url", "origin", remote_url], root, timeout=30)
            if not set_remote["ok"]:
                set_remote = _run(["git", "remote", "add", "origin", remote_url], root, timeout=30)
            attempts.append({"strategy": 2, "name": "existing-repository-origin", "result": set_remote})
            if set_remote["ok"]:
                push = _run(["git", "push", "-u", "origin", "HEAD:main"], root, timeout=240)
                attempts.append({"strategy": 2, "name": "existing-repository-push", "result": push})
                if push["ok"]:
                    verify = _run(["git", "ls-remote", "--heads", "origin", "main"], root, timeout=60)
                    attempts.append({"strategy": 2, "name": "existing-repository-verification", "result": verify})
                    if verify["ok"] and verify["stdout"].strip():
                        return {"ok": True, "passed_strategy": 2, "repository": name, "attempts": attempts}
    else:
    ...
    attempts.append({"strategy": 4, "name": "safe-handoff", "result": {
        ...
        "stderr": "GitHub publish could not be verified. Check the repository URL and GitHub authentication/permission. No source reset or delete was performed."
    }})
